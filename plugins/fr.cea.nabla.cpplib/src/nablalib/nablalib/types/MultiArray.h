...
// Meaning T1 and T2 are arithmetic type (i.e. integral or floating point) and T2
// can be converted into T1
...
// Edit: Supress the inheritence with std::vector to have a simple C array that
// is stack allocated
template <typename T, size_t DIM_1, size_t... DIM_N>
struct MultiArray : public std::array<MultiArray<T, DIM_N...>, DIM_1>
{
    ...
    template <typename ScalarT, typename BinaryOp>
    MultiArray&
    scalarOpInPlace(ScalarT x, BinaryOp op)
    {
    ...
    template <typename ArrayT, typename BinaryOp>
    MultiArray&
    arrayOpInPlace(ArrayT a, BinaryOp op)
    {
        std::transform(this->begin(), this->end(), a.begin(), this->begin(),
        [&](auto& i, auto& j) {
            return i.arrayOpInPlace(j, op);
        });
        return *this;
    }
...
struct MultiArray<T, DIM> : public std::array<T, DIM>
{
    ...
    scalarOp(ScalarT x, BinaryOp op) const
    {
        ...
        std::transform(this->begin(), this->end(), result.begin(),
        [&](auto& i) {
            return op(static_cast<RES_TYPE(T, ScalarT)>(i), x);
        });
        return result;
    }
    // Array operations
    ...
    arrayOp(ArrayT a, BinaryOp op) const
    {
        ...
        std::transform(this->begin(), this->end(), a.begin(), result.begin(),
        [&](auto& i, auto& j) {
            return op(static_cast<T>(i), static_cast<T>(j));
        });
        return result;
    }
