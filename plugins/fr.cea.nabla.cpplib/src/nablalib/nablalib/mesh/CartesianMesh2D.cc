...
namespace nablalib::mesh
{
          ...
          , m_nb_y_quads(left_nodes_ids.size() - 1)
    {
        ...
        for (size_t edgeId(0); edgeId < edges.size(); ++edgeId) {
            ...
            // Top boundary faces
            if (edgeId >= 2 * m_nb_x_quads * m_nb_y_quads + m_nb_y_quads)
                m_top_faces.emplace_back(edgeId);

            // Bottom boundary faces
            if ((edgeId < 2 * m_nb_x_quads) && (edgeId % 2 == 0))
                m_bottom_faces.emplace_back(edgeId);

            // Left boundary faces
            if ((edgeId % (2 * m_nb_x_quads + 1) == 1) &&  (edgeId < (2 * m_nb_x_quads + 1) * m_nb_y_quads))
                m_left_faces.emplace_back(edgeId);

            // Right boundary faces
            if (edgeId % (2 * m_nb_x_quads + 1) == 2 * m_nb_x_quads)
                m_right_faces.emplace_back(edgeId);
        ...
        for (size_t cellId(0); cellId < cells.size(); ++cellId) {
            ...
            // Top boundary cells
            if (i == m_nb_y_quads - 1)
                m_top_cells.emplace_back(cellId);

            // Bottom boundary cells
            if (i == 0)
                m_bottom_cells.emplace_back(cellId);

            // Left boundary cells
            if (j == 0)
                m_left_cells.emplace_back(cellId);

            // Right boundary cells
            if (j == m_nb_x_quads - 1)
                m_right_cells.emplace_back(cellId);
        }
    }
...
CartesianMesh2D::getCellsOfFace(const Id& faceId) const
{
    ...
    return BoundedArray<Id, CartesianMesh2D::MaxNbCellsOfFace>::fromVector(cells);
}

BoundedArray<Id, CartesianMesh2D::MaxNbNeighbourCells>
CartesianMesh2D::getNeighbourCells(const Id& cellId) const
{
    ...
    if (i >= 1)
        neighbours.emplace_back(index2IdCell(i - 1, j));

    if (i < m_nb_y_quads - 1)
        neighbours.emplace_back(index2IdCell(i + 1, j));

    if (j >= 1)
        neighbours.emplace_back(index2IdCell(i, j - 1));

    if (j < m_nb_x_quads - 1)
        neighbours.emplace_back(index2IdCell(i, j + 1));

    return BoundedArray<Id, MaxNbNeighbourCells>::fromVector(neighbours);
}

BoundedArray<Id, CartesianMesh2D::MaxNbFacesOfCell>
CartesianMesh2D::getFacesOfCell(const Id& cellId) const
{
    ...
    vector<Id> ret = {bottom_face, left_face, right_face, top_face};

    return BoundedArray<Id, MaxNbFacesOfCell>::fromVector(ret);
}
...
{
    auto cells = getCellsOfFace(faceId);
    if (cells.size() < 2) {
        stringstream msg;
...
Id
CartesianMesh2D::getBottomFaceNeighbour(const Id& faceId) const
{
...
Id
CartesianMesh2D::getBottomLeftFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
...
Id
CartesianMesh2D::getBottomRightFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
    if (isVerticalEdge(face))
...
Id
CartesianMesh2D::getTopFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
    return (faceId + (2 * m_nb_x_quads + 1));
...
Id
CartesianMesh2D::getTopLeftFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
    if (isVerticalEdge(face))
...
Id
CartesianMesh2D::getTopRightFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
    if (isVerticalEdge(face))
...
Id
CartesianMesh2D::getRightFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
    return (faceId + 2);
...
Id
CartesianMesh2D::getLeftFaceNeighbour(const Id& faceId) const
{
    const Edge& face(m_geometry->getEdges()[faceId]);
    assert(isInnerEdge(face));
    return (faceId - 2);
