...
import fr.cea.nabla.ir.generator.cpp.CppGeneratorUtils.DIRECTION_2D
...
class OpenMpTaskInstructionContentProvider extends InstructionContentProvider
{
	...
	private def takeOMPTraces(Set<Variable> ins, Set<Variable> outs)
	'''
		«IF OMPTraces»
		// #pragma omp atomic
		this->task_id++;
		// (UNIQ_task_id, ['in', 'in', ...], ['out', 'out', ...], start, duration) // with DAG-dot
		std::cerr << "('task_" << this->task_id << "', [«
	...
	{
		var List<DIRECTION_2D> neighbors = new ArrayList()
		var List<String> names = new ArrayList()
	...
	'''
		{
			«val parentJob = EcoreUtil2.getContainerOfType(it, Job)»
			«takeOMPTraces(ins, outs)»
			const size_t ITERATOR_LIMIT = ___partition->NEIGHBOR_getNumberForPartition(«partitionId»);
			#pragma omp task«
