...
{
	#pragma omp taskloop shared(outgoingFlux)
...
{
	#pragma omp taskloop shared(surface)
...
{
	#pragma omp taskloop shared(center)
	for (size_t jCells=0; jCells<nbCells; jCells++)
	{
		const Id jId(jCells);
...
{
	#pragma omp taskloop shared(u_nplus1)
	for (size_t jCells=0; jCells<nbCells; jCells++)
	{
...
void HeatEquation::executeTimeLoopN() noexcept
{
	...
	do
	{
		...
		// @1.0
		#pragma omp task
		computeOutgoingFlux(); // New task computeOutgoingFlux@1.0
		#pragma omp task
		computeTn(); // New task computeTn@1.0
		#pragma omp taskwait
		// @2.0
		#pragma omp task
		computeUn(); // New task computeUn@2.0
		#pragma omp taskwait
...
void HeatEquation::simulate()
{
	...
	// @1.0
	#pragma omp task
	computeSurface(); // New task computeSurface@1.0
	#pragma omp task
	computeV(); // New task computeV@1.0
	#pragma omp task
	iniCenter(); // New task iniCenter@1.0
	#pragma omp task
	iniF(); // New task iniF@1.0
	#pragma omp task
	iniTime(); // New task iniTime@1.0
	#pragma omp taskwait
	// @2.0
	#pragma omp task
	iniUn(); // New task iniUn@2.0
	#pragma omp task
